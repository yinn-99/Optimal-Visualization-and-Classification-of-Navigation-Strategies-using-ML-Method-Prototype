# PCA model for dataset (62participants, 44NSQ)
...
# loop the model to check for average accuracy
num_iterations = 20
total = 0
for i in range(num_iterations):
    ...
    Xa_train = pca.fit_transform(Xa_train)
    Xa_test = pca.transform(Xa_test)
...
uniq = np.unique(y_set)

for i, j in enumerate(uniq):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c=ListedColormap(("red", 'green', 'blue'))(j-1), marker=marker1[j-1], label=label1[j-1])
...
with open('train_Set_Output.csv', 'w') as file:
    ...
    writer.writerow(["Subject ID", "PCA Dimension 1", "PCA Dimension 2"])
    k = 1 #index number
...
for i, j in enumerate(uniq):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c=lcm[j-1], marker=marker1[j-1], label=label1[j-1])
